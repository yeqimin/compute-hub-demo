...
const filtersDirty = ref(false), createDialog = ref(false), createSubmitting = ref(false), actionDialog = ref(false), actionSubmitting = ref(false), detailVisible = ref(false), detailLoading = ref(false), detail = ref<Row | null>(null), batchResult = ref<BatchActionResult | null>(null)
...
const openCreate = () => { form.tenantId = auth.isAdmin ? form.tenantId : undefined; form.productId ||= products.value[0]?.id; form.clusterId ||= clusters.value[0]?.id; form.name ||= 'gpu-instance'; form.scenario = 'SUCCESS'; createDialog.value = true }
const create = async () => { createSubmitting.value = true; try { await api.post('/instances', { ...form, tenantId: auth.isAdmin ? form.tenantId : null }, { headers: { 'Idempotency-Key': idemKey() } }); createDialog.value = false; ElMessage.success('实例申请已提交，余额已冻结'); await refresh() } finally { createSubmitting.value = false } }
...
const retry = async (taskId: number) => { await api.post(`/tasks/${taskId}/retry`, {}, { headers: { 'Idempotency-Key': idemKey() } }); ElMessage.success('已发起人工重试'); await refresh() }
...
onMounted(async () => { const requests: Promise<Option[]>[] = [api.get<Option[]>('/products'), api.get<Option[]>('/clusters')]; if (auth.isAdmin) requests.push(api.get<Option[]>('/tenants')); const values = await Promise.allSettled(requests); products.value = values[0].status === 'fulfilled' ? values[0].value : []; clusters.value = values[1].status === 'fulfilled' ? values[1].value : []; tenants.value = values[2]?.status === 'fulfilled' ? values[2].value : []; await query.load(); timer = window.setInterval(poll, 5000) })
...
<template>
  <div class="panel instances-page">
    ...
    <InstanceDetailDrawer v-model="detailVisible" :detail="detail" :loading="detailLoading" @retry="retry" />
  </div>
</template>
<style scoped>
