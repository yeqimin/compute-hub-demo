...
onMounted(async () => { const requests: Promise<any>[] = [api.get('/products', { params: { page: 1, size: 100 } }), api.get<Option[]>('/clusters')]; if (auth.isAdmin) { tenantLoading.value = true; requests.push(api.get('/tenants', { params: { page: 1, size: 100 } })) }; const values = await Promise.allSettled(requests); products.value = values[0].status === 'fulfilled' ? (values[0].value.items || values[0].value) : []; clusters.value = values[1].status === 'fulfilled' ? values[1].value : []; tenants.value = values[2]?.status === 'fulfilled' ? (values[2].value.items || values[2].value) : []; tenantLoadError.value = auth.isAdmin && values[2]?.status === 'rejected'; tenantLoading.value = false; await query.load(); timer = window.setInterval(poll, 5000) })
