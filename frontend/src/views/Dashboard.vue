...
const capacityElement = ref<HTMLElement>(); let capacityChart: ECharts | undefined; let timer: number | undefined; let requestActive = false
...
const sample = () => { if (!metrics.value.length) return; const value = Math.round(metrics.value.reduce((sum, item) => sum + Number(item.gpuUtilization || 0), 0) / metrics.value.length * 10) / 10; samples.value = [...samples.value.slice(-11), { label: new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit', second: '2-digit' }), value }] }
const renderCapacity = () => { if (!capacityElement.value) return; capacityChart ??= init(capacityElement.value); capacityChart.setOption({ tooltip: { trigger: 'item' }, legend: { bottom: 0 }, series: [{ type: 'pie', radius: ['52%', '72%'], center: ['50%', '43%'], label: { show: false }, data: [{ name: '已分配', value: capacity.value.allocated, itemStyle: { color: '#5575ff' } }, { name: '可用', value: capacity.value.available, itemStyle: { color: '#dce6f1' } }] }] }) }
...
const toInstances = (status: string) => router.push({ path: '/instances', query: { status } })
const toTasks = (state: string) => router.push({ path: '/tasks', query: { state } })
onMounted(() => { void load(); timer = window.setInterval(load, 5000) })
onBeforeUnmount(() => { if (timer !== undefined) window.clearInterval(timer); capacityChart?.dispose() })
</script>
<template>
  <div class="dashboard">
    ...
    <template v-else-if="summary">
      ...
      <section class="grid dashboard__kpis">
        <MetricCard data-test="gpu-total" label="GPU 总量 / 可用" :value="`${numeric(summary.gpuTotal)} / ${numeric(summary.gpuAvailable)} 卡`" :hint="`运行已占用 ${numeric(summary.gpuAllocated)} 卡`" />
        ...
        <MetricCard data-test="running-instances" label="运行实例" :value="numeric(summary.runningInstances)" :hint="`异常实例 ${numeric(summary.abnormalInstances)}`" @click="toInstances('RUNNING')" />
        <MetricCard data-test="task-success-rate" label="24 小时任务成功率" :value="percent(summary.taskSuccessRate)" :hint="`异常任务 ${numeric(summary.abnormalTasks)}`" :tone="Number(summary.abnormalTasks) ? 'danger' : 'success'" @click="toTasks('UNKNOWN')" />
        <MetricCard data-test="abnormal-tasks" label="异常任务" :value="numeric(summary.abnormalTasks)" hint="FAILED / UNKNOWN / DEAD" tone="danger" @click="toTasks('UNKNOWN')" />
        <MetricCard data-test="wallet-available" label="钱包可用 / 冻结" :value="money(summary.availableCent)" :hint="`冻结 ${money(summary.frozenCent)}`" />
      </section>
      <section class="grid dashboard__charts"><article class="panel"><div class="panel-title"><h3>GPU 利用率</h3><span class="muted">浏览器会话实时采样</span></div><MetricTrend :points="samples" /></article><article class="panel"><div class="panel-title"><h3>GPU 容量</h3><span class="muted">已分配 / 可用</span></div><div ref="capacityElement" class="dashboard__capacity" data-test="capacity-chart" /></article><article class="panel"><div class="panel-title"><h3>实例状态分布</h3><span class="muted">当前范围</span></div><div v-if="summary.instanceDistribution?.length" class="distribution"><button v-for="item in summary.instanceDistribution" :key="item.status" type="button" @click="toInstances(item.status)"><span>{{ item.status }}</span><b>{{ numeric(item.value) }}</b></button></div><el-empty v-else description="暂无实例" :image-size="64" /></article></section>
      <section class="grid dashboard__bottom"><article class="panel"><div class="panel-title"><h3>集群健康</h3><span class="live"><i />{{ metrics.length ? '轮询正常' : '暂无指标' }}</span></div><div v-if="metrics.length" class="cluster-list"><div v-for="metric in metrics" :key="metric.clusterCode" class="cluster-list__item"><b>{{ metric.clusterCode }}</b><el-progress :percentage="Number(metric.gpuUtilization || 0)" :stroke-width="8" /><span>CPU {{ percent(metric.cpuUtilization) }} · 内存 {{ percent(metric.memoryUtilization) }}</span></div></div><el-empty v-else description="暂无集群实时指标" :image-size="64" /></article><article class="panel"><div class="panel-title"><h3>紧凑集群拓扑</h3><span class="muted">{{ numeric(summary.clusterCount) }} 个集群</span></div><ClusterTopology :clusters="summary.topology || []" /></article></section>
    </template>
  </div>
</template>
