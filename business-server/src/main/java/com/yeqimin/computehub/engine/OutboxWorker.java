...
public class OutboxWorker {
  ...
  @Transactional public void dispatch(){
    ...
    try{var accepted=engine.create(event);if(!accepted.getAccepted())throw new IllegalStateException(accepted.getMessage());tasks.outboxWaiting(outbox);tasks.taskWaiting(task);tasks.restoreExecuting(instance,task,executing(event));}
  ...
  @Transactional public void expireCallbacks(){
    for(Map<String,Object> row:tasks.expiredTasks()){
      ...
      if(RetryPolicy.exhausted(retry+1)){tasks.taskUnknown(task);tasks.outboxDead(outbox);tasks.instanceUnknown(instance,task,executing(row));}else{int delay=RetryPolicy.delaySeconds(retry+1);tasks.retryExpired(task,delay);tasks.retryOutbox(outbox,delay);}
    }
  }
