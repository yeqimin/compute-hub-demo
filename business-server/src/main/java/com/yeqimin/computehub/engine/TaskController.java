...
public class TaskController {
  ...
  @PostMapping("/{id}/retry") @PreAuthorize("hasAuthority('instance:retry')") @Transactional public ApiResponse<?> retry(@PathVariable long id){Map<String,Object> task=instances.task(id);if(task==null)throw BusinessException.notFound("任务不存在");UserPrincipal u=CurrentUser.get();if(!u.platformAdmin()&&num(task,"tenantId")!=u.tenantId())throw BusinessException.notFound("任务不存在");var state=engine.status(String.valueOf(task.get("commandId")));if(Set.of("RUNNING","STOPPED","DELETED","FAILED").contains(state.getStatus())){InstanceOperation operation=InstanceOperation.valueOf(String.valueOf(task.get("operation")));String result="FAILED".equals(state.getStatus())?"FAILED":"SUCCEEDED";String instanceState=state.getInstanceState().isBlank()?state.getStatus():state.getInstanceState();return ApiResponse.ok(settlement.settle(new EngineEventRequest("RECON-"+UUID.randomUUID(),state.getCommandId(),operation,result,InstanceStatus.valueOf(instanceState),state.getEngineInstanceId(),"人工对账"),UUID.randomUUID().toString().replace("-","")));}if(!"UNKNOWN".equals(task.get("taskState")))throw BusinessException.conflict("任务当前无需人工重试");tasks.manualRetry(id);tasks.manualOutbox(num(task,"instanceId"));tasks.dispatching(num(task,"instanceId"));return ApiResponse.ok(Map.of("status","RETRYING"));}
