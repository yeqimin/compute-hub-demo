...
public class BillingService {
  ...
  public Map<String,Object> ledgers(Long requestedTenant,int page,int size){long tenant=scope(requestedTenant);int safeSize=Math.min(Math.max(size,1),100),safePage=Math.max(page,1);return Map.of("items",billing.ledgers(tenant,(safePage-1)*safeSize,safeSize),"total",billing.ledgerCount(tenant),"page",safePage,"size",safeSize);}
