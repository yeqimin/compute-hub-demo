...
public class InstanceService {
  ...
  public Map<String,Object> list(String status,int page,int size){UserPrincipal user=CurrentUser.get();Long tenant=user.platformAdmin()?null:user.tenantId();int safeSize=Math.min(Math.max(size,1),100),safePage=Math.max(page,1);var items=mapper.instances(tenant,status,(safePage-1)*safeSize,safeSize);return Map.of("items",items,"total",mapper.instanceCount(tenant,status),"page",safePage,"size",safeSize);}
